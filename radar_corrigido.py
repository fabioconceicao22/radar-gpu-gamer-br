...
# Versão estável com automação opcional via CSV
# ============================================================
...
def normalizar_preco(valor):
    """
    Aceita preço em vários formatos:
    1599.90
    1.599,90
    R$ 1.599,90
    """
...
def carregar_dados():
    """
    Tenta carregar data/precos_coletados.csv.
    Se o CSV não existir, estiver inválido ou incompleto, usa os dados fixos.
    """

    caminho_csv = "data/precos_coletados.csv"
    df_fixo = pd.DataFrame(gpus)

    if not os.path.exists(caminho_csv):
        st.sidebar.info("ℹ️ Usando dados fixos")
        return df_fixo
    ...
    try:
        ...
        df_auto.columns = [str(col).strip() for col in df_auto.columns]
        ...
        renomear = {
            ...
            "Coletado em": "Data Coleta"
        }

        df_auto = df_auto.rename(columns=renomear)

        colunas_minimas = ["GPU", "Link", "Preco Atual"]

        for coluna in colunas_minimas:
            if coluna not in df_auto.columns:
                st.sidebar.warning("⚠️ CSV automático incompleto. Usando dados fixos.")
                return df_fixo

        if "Loja" not in df_auto.columns:
            df_auto["Loja"] = df_auto["Link"].apply(identificar_loja)

        df_auto["Preco Atual"] = df_auto["Preco Atual"].apply(normalizar_preco)
        df_auto = df_auto.dropna(subset=["Preco Atual"])

        if df_auto.empty:
            st.sidebar.warning("⚠️ CSV sem preços válidos. Usando dados fixos.")
            return df_fixo

        colunas_tecnicas = [
            "GPU", "Modelo", "Marca", "Preco Antigo", "VRAM",
            "FPS 1080p", "FPS 1440p", "Streaming Score",
            "Gamer Score", "Consumo W"
        ]

        df = pd.merge(
            df_auto[["GPU", "Loja", "Preco Atual", "Link"]],
            df_fixo[colunas_tecnicas],
            on="GPU",
            how="left"
        )

        df = df.dropna(subset=["Modelo", "Marca", "VRAM"])

        if df.empty:
            st.sidebar.warning("⚠️ CSV não bateu com a base fixa. Usando dados fixos.")
            return df_fixo

        st.sidebar.success("✅ Dados automáticos carregados")
        return df

    except Exception as erro:
        st.sidebar.warning("⚠️ Erro ao ler CSV automático. Usando dados fixos.")
        return df_fixo
